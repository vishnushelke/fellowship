	...
	public static void main(String[] args) throws JsonGenerationException, JsonMappingException, IOException {
		...
		while (!exit) {
			...
			case 2:// buy shares
				...
				System.out.println("this company has " + company.getNumberofshares() + " shares and shareprice "
				...
				// code for saving transactions into transaction array list

				DateFormat dateFormat = new SimpleDateFormat("dd/MM/yy HH:mm:ss");
				Date date = new Date();
				Transaction transaction = new Transaction();
				transaction.setDate(dateFormat.format(date));
				transaction.setPaidby(customer.getName());
				transaction.setPaidto(company.getCompanyname());
			...
			case 3:// sell shares
				...
				while (shareNumber > customer.getAvailableshares()) {
					System.out.println("you have only " + customer.getAvailableshares()
							+ " in your account,enter valid number of shares to be sold");
				...
				transaction.setPaidby(customer.getName());
				transaction.setPaidto(company.getCompanyname());
				transaction.setPaidmoney(company.getShareprice()*shareNumber);
				transaction.setStocksold(shareNumber);
				transactions.add(transaction);
		...
		modelTransaction.setTransactions(transactions);
		mapper.writeValue(new File("/home/user/Desktop/vishnu/bridgelabzJavaPrograms/javaprograms/bootcamp/Files/json files/stocktransactions.json"), modelTransaction);
